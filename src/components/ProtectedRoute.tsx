...
interface ProtectedRouteProps {
  ...
  // When true, show skeleton immediately instead of a brief spinner delay
  immediateSkeleton?: boolean;
}

export default function ProtectedRoute({
  children,
  showSkeleton = true,
  immediateSkeleton = false,
}: ProtectedRouteProps) {
  ...
  const shouldShowSkeleton = immediateSkeleton || showDelayedSkeleton;

  // Show loading while checking authentication
  if (loading) {
    if (showSkeleton && shouldShowSkeleton) return SkeletonScreen;
